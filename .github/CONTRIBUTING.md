...
- Before submitting a bug report test using the [latest version of the project](https://github.com/per1234/inolibbuglist/archive/master.zip) to be sure it hasn't already been fixed. **Don't report issues that only occur with old versions of the project.**
- Search [existing pull requests and issues](https://github.com/per1234/inolibbuglist/issues?q=) to be sure it hasn't already been reported. **Do not submit duplicate issue reports.** If you have additional information to provide about the issue then please comment on that issue.
- Open an issue at https://github.com/per1234/inolibbuglist/issues/new.
...
## Pull Requests
- Search [existing pull requests and issues](https://github.com/per1234/inolibbuglist/issues?q=) to make sure the change hasn't already been proposed.
- Code formatting should be consistent with the current code.
- Unit tests for the script are located in the `tests` folder: `python test_inolibbuglist.py --ghtoken <GITHUBTOKEN>`
  - `--verbose` command line argument provides verbose output for debugging.
- Update the [documentation](https://github.com/per1234/inolibbuglist/blob/master/README.md) if your changes require it. This should be done in the same commit as the change.
...
- Open a pull request at https://github.com/per1234/inolibbuglist/compare.
